...
path = "/home/public/cb/code/lafin/checkpoints/celebahq-ae-lmfaceidin-256-512-latent_refine4"
...
# discriminator = Discriminator(image_size, network_capacity, fmap_max = 512)
...
for epoch in range(start_iteration,10000):
    ...
    for items in train_loader:
        ...
        ref_outputs = Inpaintor(images, landmark_map, masks)
        
        coarse = torch.cat((ref_outputs-images,outputs,landmark_map),dim=1) 
        ...
        dis_loss = 0
        dis_input_real1 = images
        dis_input_real2 = ref_images
        dis_input_fake = refine_result.detach()
 
        #dis_real1, _ = discriminator(torch.cat((dis_input_real1, landmark_map), dim=1))                   # in: [rgb(3)+landmark(1)]
        #dis_real2, _ = discriminator(torch.cat((dis_input_real2, ref_landmark_map), dim=1))
        dis_real2, _ = discriminator(torch.cat((dis_input_real2, ref_landmark_map), dim=1))
        dis_real = dis_real2
