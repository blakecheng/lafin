...
if __name__ == "__main__": 
    ...
    # img_path = os.path.join(source_img_dir,img_list[456])
    # demo_input = api.get_input(img_path,mask_face=True)
    # images,landmark_map,masks, face, landmark_img , raw_mask = demo_input

    # show_list = []
    # output = api.gen_result(demo_input[0].cuda(),demo_input[1].cuda(),demo_input[2].cuda())
    # # mask = target_inputs[2].cuda() 
    # # output = target_inputs[0].cuda()* (1-mask) + output* mask
    # show_result = np.clip(np.transpose(output[0].detach().cpu().numpy(),(1,2,0)),0,1)
    # show_list.append(show_result*255)
    # img = stitch_images(show_list,256,1)
    # display(img)

    # show_list = []
    # for i in range(10):
    #     re_landmarks = np.transpose(face3d.roate(cv2.resize(face,(INPUT_SIZE,INPUT_SIZE)),yaw_pose=(i*4-10), pitch_pose=0, roll_pose=0)[0],(1,0))
    ...
    #     output = api.gen_result(demo_input[0].cuda(),landmark_map.cuda(),demo_input[2].cuda())
    #     # mask = target_inputs[2].cuda() 
    #     # output = target_inputs[0].cuda()* (1-mask) + output* mask
    #     show_result = np.clip(np.transpose(output[0].detach().cpu().numpy(),(1,2,0)),0,1)
    #     show_list.append(show_result*255)

    # img = stitch_images(show_list,256,10)
    # display(img)
