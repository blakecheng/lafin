...
class Generator(nn.Module):
    ...
    def forward(self, x, styles, input_noise=None):
...
class Trainer():
    def __init__(self, name, results_dir, models_dir, image_size, network_capacity, transparent = False, batch_size = 4, mixed_prob = 0.9, gradient_accumulate_every=1, lr = 2e-4, num_workers = None, save_every = 1000, trunc_psi = 0.6, fp16 = False, cl_reg = False, fq_layers = [], fq_dict_size = 256, attn_layers = [], no_const = False, aug_prob = 0., dataset_aug_prob = 0.,local_rank = -1, *args, **kwargs):
        ...
        self.lm_detector = MobileNetV2(points_num=68).cuda()
    ...
    def init_GAN(self):
        ...
        self.GAN = torch.nn.parallel.DistributedDataParallel(self.GAN, device_ids=[self.local_rank])
    ...
    def set_data_src(self, folder):
        ...
        self.loader = cycle(data.DataLoader(self.dataset, num_workers = default(self.num_workers, num_cores), batch_size = self.batch_size, drop_last = True, shuffle=False, pin_memory=True,sampler=self.train_sampler))
    ...
    def train(self):
        ...
        for i in range(self.gradient_accumulate_every):
            ...
            with torch.no_grad():
                resize_img = F.interpolate(image_batch, [256, 256], mode='bilinear', align_corners=True)
                landmarks = self.lm_detector(resize_img)
                landmarks = landmarks.reshape(batch_size,68,2)
                landmarks = (landmarks*256).long()
                landmarks[landmarks >= 256] = 256 -1
                landmarks[landmarks < 0] = 0
                landmark_map = torch.zeros((batch_size,1,256,256)).cuda()
                for i in range(landmarks.shape[0]):
                    landmark_map[i,0,landmarks[i,0:68,1],landmarks[i,0:68,0]] = 1
                landmark_image = F.interpolate(landmark_map, [image_size, image_size], mode='bilinear', align_corners=True)
                
            
            generated_images = self.GAN.G(landmark_image, w_styles)
    ...
    def generate_truncated(self, S, G, style, noi, trunc_psi = 0.75, num_image_tiles = 8):
        ...
        generated_images = evaluate_in_chunks(self.batch_size, G, landmark_image, w_styles)
