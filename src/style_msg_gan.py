...
class MSG_STYLE_Generator(th.nn.Module):
    ...
    def __init__(self, depth=7, latent_size=512, use_eql=True, n_mlp=8, base_channel=512):
        ...
        from .style_msg_gan_customlayers import GenGeneralConvBlock, PixelNorm, EqualConv2d, EqualLinear
        ...
        self.layers = ModuleList([GenGeneralConvBlock(in_channels=base_channel, out_channels=base_channel, fused=False, initial=True)])
        self.rgb_converters = ModuleList([to_rgb(base_channel)])

        # create the remaining layers
        for i in range(self.depth - 1):
            if i <= 2:
                layer = GenGeneralConvBlock(in_channels=base_channel, out_channels=base_channel,
                                            fused=False, initial=False)
                rgb = to_rgb(base_channel)
            else:
                layer = GenGeneralConvBlock(int(base_channel // np.power(2, i - 3)),
                                            int(base_channel // np.power(2, i - 2)), fused=False, initial=False)
...
class Discriminator(th.nn.Module):
    ...
    def __init__(self, depth=7, feature_size=512,
        ...
        from .CustomLayers import DisGeneralConvBlock, \
            DisFinalBlock, EqualConv2d
