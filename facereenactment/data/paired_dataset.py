...
class PairedDataset(BaseDataset):
    ...
    def modify_commandline_options(parser, is_train):
        parser.set_defaults(dataroot='/data/chengbin/dataset/head2head/dataset/train')
        # parser.add_argument('--dataroot', type=str, default='/data/chengbin/dataset/head2head/dataset/train')
    ...
    def initialize(self, opt):
        ...
        if opt.isTrain:            
            self.L_paths = sorted(make_grouped_dataset(path.join(root, 'landmarks'))) 
            self.I_paths = sorted(make_grouped_dataset(path.join(root, 'images')))
