...
def copy_dataset(obs_path, cache_path, mode="remote"):
    ...
    if mode == "local":
        command("cp -r {} {}".format(obs_path, cache_path))
    else:
...
def stop_task(task_list):
    ...
    config = yaml.load(f)
    f.close()
    kill_list = config['kill_list']
    
    for task in task_list:
        try:
            if task.name in kill_list :
                stop_thread(task)
                print("Stopping task: {}".format(task.name))
                task_list.remove(task)
                kill_list.remove(task)
        except Exception as e:
                print("Stopping error")
                print(e)
        finally:     
            config['task'][task.name]["finished"] = True
...
code_path = "/data/chengbin/code/workspace"
...
try:
    while True:
        ...
        if (int(end - start)) % 20 == 0:
            ...
            if config["to_stop"] == True:
                print("Prepare stop, Total Time:{:.4f} , Excuting.....".format(end - init))
                try:
                    print("Project update: \n", config)
                    print("stoping task ...")
                    task_list = stop_task(task_list)
                except Exception as e:
                    print(e)
                finally:
                    config["to_stop"] = False
                    fr = open(project_yml, 'w')
                    yaml.dump(config, fr)
                    fr.close()
                    copy_file(code_path + "/" + project_yml, s3code_path + "/" + project_yml, "local")
